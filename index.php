  ...
  const rows = customers.map(c => {
    ...
    if (expanded) {
      const ordersHtml = custSales.length ? custSales.map(s => {
        ...
        let badge = paid >= total ? `<span class="badge bg-success">Paid</span>` : (paid > 0 ? `<span class="badge bg-warning text-dark" title="Paid ${fmt(paid)} / Due ${fmt(due)}">Advance</span>` : `<span class="badge bg-danger">Unpaid</span>`);
        return `
          <div class="row-line" style="display:grid;grid-template-columns:95px 1fr 85px 95px 95px 120px 120px;gap:8px;align-items:center;">
        ...
        `;
      }).join("") : `<div class="empty-msg">No sales or orders recorded for this customer yet. Log sales from the Sales tab.</div>`;

      statementBody = `
        <div class="card-body border-top bg-light mt-2 p-3" style="border-radius:0 0 10px 10px;">
          <div class="d-flex justify-content-between align-items-center mb-3">
            <span class="fw-bold small text-dark text-uppercase">📋 Customer Statement &amp; Sales Ledger</span>
          ...
          <div class="table-panel">
            <div class="mono table-head" style="display:grid;grid-template-columns:95px 1fr 85px 95px 95px 120px 120px;gap:8px;">
...
function renderIncome() {
  ...
  return `
    <div class="panel">
      ...
      <div class="muted small" style="margin-bottom:12px">Log sales &amp; advances. Select existing customer or create new profile on-the-fly.</div>
      <div class="row g-2 align-items-center mb-2">
        ...
        <div class="col-md-5 col-12">
          <select class="form-select" id="inc-customer" data-act="inc-cust-select">
            ${customerOptions}
          </select>
        </div>
        <div class="col-md-7 col-12" id="inc-new-cust-fields" style="display:none;">
          <div class="row g-2">
            <div class="col-6"><input class="form-control" id="inc-new-cust-name" placeholder="New Customer Name *"></div>
            <div class="col-6"><input class="form-control" id="inc-new-cust-phone" placeholder="WhatsApp / Phone (e.g. 03001234567)"></div>
          </div>
        </div>
        <div class="col-12"><input class="form-control" id="inc-note" placeholder="Order Note / Special details (optional)"></div>
