...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .form-grid-emp { display:grid; grid-template-columns:1fr 1fr 150px auto; gap:8px; align-items:center; }
  ...
  .form-grid-partner { display:grid; grid-template-columns:1fr 90px 130px; gap:8px; margin-bottom:10px; }
  ...
  .expense-row { display:grid; grid-template-columns:76px 1fr 84px 140px 54px; gap:6px; align-items:center; }
  .income-row { display:grid; grid-template-columns:76px 1fr 50px 76px 100px 54px; gap:6px; align-items:center; }
...
async function showEditVendorModal(id) {
  ...
  const { value: formValues } = await Swal.fire({
    ...
    html:
      ...
      `<div><label style="font-size:12px;font-weight:600;">Note / Phone / Address</label><input id="swal-v-note" class="swal2-input" value="${esc(vendor.note||'')}" style="margin:4px 0 0 0;width:100%;"></div>` +
      `</div>`,
    focusConfirm: false,
    showCancelButton: true,
    confirmButtonColor: '#2F6F63',
  ...
  return `
    <div class="panel">
      ...
      <div id="new-emp-weekly-wrap" style="margin-top:8px">
        <input class="led-input" id="new-emp-weekly" type="number" placeholder="Weekly salary amount" style="max-width:220px">
      </div>
      <div id="new-emp-work-wrap" style="display:none;margin-top:8px" class="muted small">
        You'll add their piece-rate work items (e.g. cutting, stitching) with prices after creating their profile.
      </div>
    </div>
